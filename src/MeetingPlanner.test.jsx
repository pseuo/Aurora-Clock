...
function TestPlanner() {
  ...
  return (
    <>
      <button type="button" onClick={() => setIsOpen(true)}>Open planner</button>
      {isOpen && <MeetingPlanner language="en" labels={copy.en} onClose={() => setIsOpen(false)} />}
    </>
  );
}
...
describe('MeetingPlanner', () => {
  it('focuses, traps focus, and closes with Escape', async () => {
    ...
    const dialog = await screen.findByRole('dialog', { name: 'Time zone meeting' });
  ...
  it('handles a cleared date and calculates quarter-hour time-zone boundaries', () => {
    render(<MeetingPlanner language="en" labels={copy.en} onClose={() => {}} />);
    fireEvent.change(screen.getByLabelText('日期 / 时间'), { target: { value: '' } });
    expect(screen.getAllByText('—')).toHaveLength(3);

    const mumbai = worldClocks.find((city) => city.id === 'mumbai');
    const dubai = worldClocks.find((city) => city.id === 'dubai');
    expect(overlapFor(new Date('2026-08-17T12:00:00'), mumbai, dubai, 'en-US')).toBe('10:30 – 18:00');
  });
});
