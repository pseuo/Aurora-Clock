...
describe('weather state', () => {
  ...
  it('maps a successful weather response to a ready state', async () => {
    Object.defineProperty(navigator, 'geolocation', { configurable: true, value: { getCurrentPosition: (success) => success({ coords: { latitude: 35.7, longitude: 139.7 } }) } });
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        current: { time: '2026-08-17T03:00', temperature_2m: 24.6, apparent_temperature: 25.2, weather_code: 63 },
    ...
    expect(result.current).toMatchObject({ labelKey: 'rain', temp: 25, feelsLike: 25, atmosphere: 'rain' });
    expect(result.current.precipitation).toBe(42);
    expect(new URL(fetch.mock.calls[0][0]).searchParams.get('latitude')).toBe('35.7');
    expect(new URL(fetch.mock.calls[0][0]).searchParams.get('longitude')).toBe('139.7');
  ...
  it('selects precipitation using the forecast response time', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        current: { time: '2026-08-17T03:00', temperature_2m: 24, apparent_temperature: 24, weather_code: 0 },
        hourly: {
  ...
  it('reports a denied location so the interface can offer manual setup', async () => {
    ...
    await waitFor(() => expect(result.current).toMatchObject({ status: 'denied', labelKey: 'locationDenied' }));
  });
  ...
  it('uses a saved manual location without requesting browser location', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ current: { temperature_2m: 20, apparent_temperature: 19, weather_code: 0 }, hourly: { precipitation_probability: [0] } }),
    }));

    const { result } = renderHook(() => useWeather(true, true, { latitude: 31.2, longitude: 121.5, name: 'Shanghai' }));
  ...
  it('resolves a city into a manual weather location', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ results: [{ name: 'Shanghai', admin1: 'Shanghai', country: 'China', latitude: 31.2, longitude: 121.5 }] }),
    }));

    await expect(findWeatherLocation('Shanghai')).resolves.toEqual({ latitude: 31.2, longitude: 121.5, name: 'Shanghai, Shanghai, China' });
  });
});
