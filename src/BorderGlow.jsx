...
function colorFromGlowValue(glowColor) {
  const [hue = "185", saturation = "90", lightness = "78"] =
    String(glowColor).match(/[\d.]+/g) ?? [];
...
const BorderGlow = ({
  ...
  const updateGlow = useCallback(
    (event) => {
      ...
      frameRef.current = requestAnimationFrame(() => {
        ...
        const angle =
          Math.atan2(y - height / 2, x - width / 2) * (180 / Math.PI);
  ...
  const [first = fallbackColor, second = fallbackColor, third = fallbackColor] =
    colors;
    ...
    >
      ...
      <div className="relative z-10 h-full min-h-0 overflow-hidden rounded-[calc(var(--border-radius)-1px)]">
        <span
          ...
          style={{ backgroundColor, opacity: panelOpacity }}
        />
